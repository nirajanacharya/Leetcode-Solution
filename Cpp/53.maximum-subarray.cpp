...
public:
    int maxSubArray(vector<int>& nums) {
        int maxSubarraySum = nums[0], currSubarraySum = nums[0];
        int len = nums.size();
        for (int i = 1; i < len; i++) {
            currSubarraySum = std::max(nums[i], currSubarraySum+nums[i]);
            maxSubarraySum = std::max(maxSubarraySum, currSubarraySum);
        }
